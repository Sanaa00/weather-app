...
function Weather() {
  ...
  const { data: locationWeather, refetch: refetchLocationWeather } =
    useGetWeatherQuery(search)
  const { data: daysForecastBySearch, refetch: refetchForecastBySearch } =
    useGet5DayForecastQuery(search)
  const { data: daysForecast, refetch: refetchForecast } =
  ...
  useEffect(() => {
    if (currentLocation) {
      refetchCurrentWeather(currentLocation)
    }
  }, [currentLocation, refetchCurrentWeather])
  ...
  const handleHistoryClick = () => {
    // Fetch and display the history of cached locations
